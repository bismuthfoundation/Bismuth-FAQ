...
The wallet will now show the corresponding balance or transactions to this address. So if you want to change through your addresses, this can be done here.
...
Now you see the 3 latest transactions, your balance and you can send or receive BIS from here. The info about  transactions is reduced, for full info, enter the transactions page.

## Transactions

Transactions shows the last 10 tx you received or initiated yourself, with all details (txid, Date/Time, Address, Amount, Type, Fees and Data)

![Oups, where is the Screenshot?](graphics/transactions.png)

If you mouse over **Txid**, the txid of this transaction is shown. There is actually no way to copy it and paste it in a ticket or search for that id on explorer. The **Type** field shows the bismuth protocol operation, you performed with your transaction (this needs some more explanation and an additional faq-section for this will be added later). As a short explanation, type (operation field in legacy wallet) is used if you transfer token or other special functions.
...
On the right side, you see the actual block and the mining difficulty of the Bismuth-Network. Below this, you have a list of available wallet servers. This list is generated by an API-call. You can refresh this list by clicking **Refresh from API**. If you click on an **IP address**, the wallet will try to connect to this wallet server. On startup, the wallet automatically uses the wallet server which is loaded the least to not overload one server. 

The displayed block height is also received from API. It represents the block, when the wallet servers where checked last time, so this will very often differ from actual block you see above. It is shown to you, so you as a user can see, if a wallet server seems to have issues (some blocks behind for example).

## Crystals

Crystals are the tornado-based wallet equivalent for addons.

![Oups, where is the Screenshot?](graphics/crystals.png)

Actually, 4 addons are already integrated. If you activate them by clicking the checkbox, an submenu gets created, where you can see details of the addon. Some crystals are shown on dashboard (bismuthprice for example), others are working in the page behind the link in the submenu (dragginator for example).
...
If you want to sent BIS, you can reach the dialogue from dashboard, if you have selected an active address or from transaction-page.

![Oups, where is the Screenshot?](graphics/send.png)

On the left side of the main window, you have to enter the address of the recipient. You can optionally enter data (a text message or in case of sending to some exchanges, the id, they gave you in their deposit dialogue). Another option, hiding behind the dropdown-arrow is a BIS-operation.

On the right side, you have to enter the amount of BIS, you want to send. The wallet calculates the fee and shows the total value, that will be sent after you click on **send BIS**.

Last option is: you received a BIS-URL. Click the dropwdown-arrow, paste the BIS-URL and click **send BIS-URL**.

Both times clicking send will open the confirm dialogue:

![Oups, where is the Screenshot?](graphics/confirm.png)

With **Confirm and send BIS**, your transaction will be broadcasted to the network.



## Receive BIS

The receive page is used ro create the formerly mentioned BIS-URL.

![Oups, where is the Screenshot?](graphics/receive.png)

The page is similar to the one, that is used to send BIS. Left side, ***your*** address is prefilled, because you want to create a BIS-URL to give somebody, so they can sent BIS to you. You can add data, that is also integrated into the BIS-URL. The qr-code represents just your address. You can copy the picture and leave or send it somewhere, instead of giving them your address.

On the right sid, you can enter the amount, you want to receive.
...
For improved security, you can set a master password (see on [Screenshot](# first-start) how it looks like). Setting one will encrypt your **wallet.json**. So loosing your master password means loss of your funds. Store it in a save place. Due to the added security, the wallet will get slower on some operations, because in background, there has to be some decryption processes then. If you start a master-password-protected wallet, you have to always unlock it first.

## Spend Protection

When you added a **master password** you can add spend protection to your wallet. Actually only PIN is supported.

![Oups, where is the Screenshot?](graphics/spend_protection.png)

After you unlocked the wallet, you see a new field on the right part of the main window, called spend protection. You have to enter a PIN Code and your master password to activate the protection. You can also deactivate it later, if you want to. After clicking **Set spend protection**, every dialogue, that needs your confirmation to send BIS, will also need that PIN in order to be confirmed.
